...
function GBC_emulator_core()
{
   ...
   this.buttons = { a: false, b: false, start: false, select: false, 
   ...
   this.cpu_speed = 0; // 0 = normal, 1 = CGB double speed
}
...
{
   // The speed switch unlock is only allowed if the game claims Color support,
   //  so even though we don't check that here, there's no way to switch the speed
   //  on a regular Game Boy (DMG) game.
...
GBC_emulator_core.prototype.mem_read = function(address)
{
   ...
   else if (address === 0xff00)
   {
      ...
      return this.timer.reg_read(address);
   else if (address === 0xff0f)
      return this.interrupt_flags;
   else if (address === 0xff46)
      return 0;
   else if (address === 0xff4d)
      return this.speed_switch_unlocked | (this.cpu_speed << 7);
...
GBC_emulator_core.prototype.mem_write = function(address, value)
{
   ...
   if ((address >= 0x8000) && (address < 0xa000))
      ...
      this.keypad_nibble_requested = (value >>> 5) % 2;
   else if ((address >= 0xff04) && (address <= 0xff07))
      this.timer.reg_write(address, value);
   else if (address === 0xff0f)
      this.interrupt_flags = value;
   else if (address === 0xff46)
   {
      var start_address = value << 8;
      if ((start_address < 0xa000) && (start_address >= 0x8000))
         this.video.do_oam_dma(start_address);
      else
         this.video.do_oam_dma(this.memory.get_dma_array(start_address, 160));
   }
   else if (address === 0xff4d && this.memory.cgb_game)
      this.speed_switch_unlocked = value % 2;
   else if (address === 0xff51 && this.memory.cgb_game && !this.vram_dma_running)
   ...
   {
      if (this.vram_dma_source > 0xdff0 ||
          (this.vram_dma_source > 0x7ff0 && this.vram_dma_source < 0xa000))
         return;
      
      var length = ((value & 0x7f) + 1) * 16;
      this.video.do_vram_dma(this.memory.get_dma_array(this.vram_dma_source, length),
                             this.vram_dma_destination, (value & 0x80) >>> 7);
   }
   else if ((address >= 0xff40) && (address <= 0xff4b))
      this.video.mem_write(address, value);
   else if ((address >= 0xff68) && (address <= 0xff6b))
