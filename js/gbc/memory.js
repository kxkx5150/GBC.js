...
GBC_memory.prototype.load_rom = function(rom_array, filename) {
  ...
  this.is_battery_backed =
    (cartridge_type === 3) || (cartridge_type === 6) || (cartridge_type === 9) ||
    (cartridge_type === 13) || (cartridge_type === 15) || (cartridge_type === 16) ||
    (cartridge_type === 19) || (cartridge_type === 23) || (cartridge_type === 27) ||
    (cartridge_type === 30) || (cartridge_type === 255);

  if (cartridge_type === 0)
    this.mapper = this.mappers.no_mapper(this);
  else if (cartridge_type === 1 || cartridge_type === 2 || cartridge_type === 3)
    this.mapper = this.mappers.mbc1(this);
  else if (cartridge_type === 5 || cartridge_type === 6)
    this.mapper = this.mappers.mbc2(this);
  else if (cartridge_type === 15 || cartridge_type === 16 || cartridge_type === 17 ||
           cartridge_type === 18 || cartridge_type === 19)
    this.mapper = this.mappers.mbc3(this);
  else if (cartridge_type === 25 || cartridge_type === 26 || cartridge_type === 27 ||
           cartridge_type === 28 || cartridge_type === 29 || cartridge_type === 30)
    this.mapper = this.mappers.mbc5(this);
  else
    throw ("Unknown cartridge type.");

  this.cgb_game = (rom_array[0x143] === 0x80) || (rom_array[0x143] === 0xc0);

  if (this.cgb_game)
    this.working_ram = new Uint8Array(0x8000);
  else
    this.working_ram = new Uint8Array(0x2000);
  ...
  this.num_rom_banks =
    ...
    (rom_array[0x148] === 1) ? 4 :
    (rom_array[0x148] === 2) ? 8 :
    (rom_array[0x148] === 3) ? 16 :
    (rom_array[0x148] === 4) ? 32 :
    (rom_array[0x148] === 5) ? 64 :
    (rom_array[0x148] === 6) ? 128 :
    (rom_array[0x148] === 0x52) ? 72 :
    (rom_array[0x148] === 0x53) ? 80 :
    (rom_array[0x148] === 0x54) ? 96 : 2;
...
GBC_memory.prototype.detect_local_storage = function() {
  // This code was taken from the MDN page on "Using the Web Storage API"
  try {
...
GBC_memory.prototype.write_save_file = function() {
  if (this.save_key && !this.save_timeout) {
    this.save_timeout = window.setTimeout((function() {
      let binary = "";
...
GBC_memory.prototype.mem_read = function(address) {
  if (address < 0x8000) {
    // Cartridge ROM. Could be switched, so let's ask the mapper.
    return this.mapper.mem_read(address);
  } else if (address < 0xc000) {
    // This is the cartridge's RAM, if it has any.
    // If we have 2kB of RAM in an 8kB bank,
    //  then we need to mirror that 2kB four times.
    if (this.ram_size === 1)
      address &= 0xe7ff;
    return this.mapper.mem_read(address);
  } else if (address < 0xd000) {
    // This is bank 0 if the internal RAM of the console.
    return this.working_ram[address - 0xc000];
  } else if (address < 0xe000) {
    // This is the selected bank of the internal RAM of the console.
    // Note that in DMG mode, the bank number is always 1.
    return this.working_ram[(this.wram_bank * 0x1000) + address - 0xd000];
  } else if (address < 0xfe00) {
    // This is a mirror of the working RAM.
    // Call ourself recursively instead of copying the code above.
    return this.mem_read(address - 0x2000);
  } else if (address === 0xff70) {
    // This is the CGB WRAM bank select register.
    return this.wram_bank;
  } else if (address >= 0xff80) {
    // This is the zero page. I imagine it's called that because
    //  of it's similarity in function to the 6502 zero page.
...
GBC_memory.prototype.mem_write = function(address, value) {
  if (address < 0x8000) {
    // Writing into the ROM; must be aiming for a mapper register.
...
GBC_memory.prototype.get_dma_array = function(start_address, length) {
  // I'm too lazy to bother putting in the interface to get
  //  an actual subarray, so we'll just build a new one the long way.
  var new_array = new Uint8Array(length);
  for (var i = 0; i < length; ++i)
    new_array[i] = this.mem_read(start_address + i);
  return new_array;
};
  ...
  return {
    mem_read: function(address) {
      if (address < 0x4000)
        ...
        return mem.cartridge_rom[(rom_bank * 0x4000) + address - 0x4000];
      else if ((address >= 0xa000) && (address < 0xc000)) {
        if (mem.num_ram_banks) {
          if (mode)
            return mem.cartridge_ram[(ram_bank * 0x2000) + address - 0xa000];
          else
            return mem.cartridge_ram[address - 0xa000];
        }
      }
    },
    ...
    mem_write: function(address, value) {
      if (address < 0x2000) {
        ...
        if (!mode)
          rom_bank = ((rom_bank & 0x60) | (value & 0x1f)) % mem.num_rom_banks;
        else
          rom_bank = (value & 0x1f) % mem.num_rom_banks;
      } else if (address < 0x6000) {
        if (!mode)
          rom_bank = ((rom_bank & 0x1f) | ((value & 0x03) << 5)) % mem.num_rom_banks;
        else
          ram_bank = (value & 0x03) % mem.num_ram_banks;
      } else if (address < 0x8000) {
        mode = value % 2;
      } else if ((address >= 0xa000) && (address < 0xc000)) {
        if (mem.num_ram_banks && cart_ram_enabled) {
          if (mode)
            ...
            mem.cartridge_ram[address - 0xa000] = value;
        }
        if (mem.is_battery_backed)
          mem.write_save_file();
      }
    }
  };
...
GBC_memory.prototype.mappers.mbc2 = function(mem) {
  var cart_ram_enabled = false,
    rom_bank = 1;
  // For the MBC2, the RAM is in the mapper chip, not elsewhere on the cartridge board.
  ...
  return {
    ...
    mem_write: function(address, value) {
      if (address < 0x2000) {
        if (!(address & 0x0100))
          cart_ram_enabled = ((value & 0xf) === 0xa);
      } else if (address < 0x4000) {
        value = (value === 0) ? 1 : value;
        if (address & 0x0100)
          rom_bank = value & 0x0f & (mem.num_rom_banks - 1);
      } else if (address >= 0xa000 && address < 0xa200) {
        if (cart_ram_enabled)
          cart_ram[address - 0xa000] = value & 0x0f;
      }
    }
  };
};
    ...
    mem_read: function(address) {
      if (address < 0x4000)
        ...
        if (mem.num_ram_banks && cart_ram_enabled && (ram_bank <= 3))
          return mem.cartridge_ram[(ram_bank * 0x2000) + address - 0xa000];
        else if (ram_bank === 0x08)
          return (new Date()).getSeconds();
        else if (ram_bank === 0x09)
          return (new Date()).getMinutes();
        else if (ram_bank === 0x0a)
  ...
  return {
    ...
    mem_write: function(address, value) {
      if (address < 0x2000)
        ...
        rom_bank = (rom_bank & 0xff) | ((value % 2) << 8);
      else if (address < 0x6000)
        ram_bank = value & 0x0f;
      else if (address >= 0xa000 && address < 0xc000) {
        if (mem.num_ram_banks && cart_ram_enabled)
