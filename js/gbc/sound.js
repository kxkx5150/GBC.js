...
const sound = {
  ...
  4:{ 
    ...
    polySteps:function(x){ sound.lfsrPhase=0;sound[4].oscillator.onaudioprocess = x?sound.processLFSR7bit:sound.processLFSR15bit; },
    bitPeriod:1,
  ...
  createBit(){
    var limit = 1,  bit = sound.FFTsize >> 1;
  ...
  setSound3Waveform() {
    ...
    var i, real = new Float32Array(sound.FFTsize), imag = new Float32Array(sound.FFTsize), samples=new Float32Array(sound.FFTsize);
    for ( i = 0; i < 16; i++ ) {
       samples[32*i+0] = 
       samples[32*i+1] = 
       samples[32*i+2] = 
    ...
    sound[3].oscillator.setPeriodicWave(
      audioCtx.createPeriodicWave(real.slice(0,sound.FFTsize/2),imag.slice(0,sound.FFTsize/2), {disableNormalization:true})
    );
