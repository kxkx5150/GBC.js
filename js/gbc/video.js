...
GBC_video.prototype.draw_line = function () {
  ...
  if (this.background_enabled || this.is_cgb) {
    for (var tile_ix = 0; tile_ix < 21; tile_ix++) {
      var tile_pos =
        (((this.current_line + this.scroll_y) & 0xff) >>> 3) * 0x20 + (this.scroll_x >>> 3) + tile_ix;
      if (this.scroll_x >= 0x60 && 19 - tile_ix < (this.scroll_x - 0x60) >>> 3) tile_pos -= 0x20;
      ...
      for (var pixel = 0; pixel < 8; pixel++) {
        ...
        else color = ((tile_data_lo >>> (7 - pixel)) & 1) | (((tile_data_hi >>> (7 - pixel)) & 1) << 1);
        ...
        if (this.is_cgb) {
          ...
          if (tile_priority) background_priority[line_index] = color !== 0;
        } else {
  ...
  if (this.window_enabled) {
    if (this.window_x < 167 && this.window_y < 144 && this.window_y <= this.current_line) {
      for (var win_pixel = this.window_x - 7; win_pixel <= 166; win_pixel++) {
        ...
        var tile_pos =
          (((this.current_line - this.window_y) & 0xff) >>> 3) * 0x20 +
        ...
        var pixel_in_tile = horizontal_flip
          ...
          : ~(win_pixel - this.window_x + 7) & 0x07;
        var color =
          ((tile_data_lo >>> pixel_in_tile) & 1) | (((tile_data_hi >>> pixel_in_tile) & 1) << 1);
        ...
        if (this.is_cgb) {
          ...
          if (tile_priority) background_priority[win_pixel] = color !== 0;
        } else {
      ...
      if (sprite_y > 0 && sprite_y < 160 && sprite_x < 168) {
        ...
        if (this.current_line >= sprite_y && this.current_line < sprite_y + this.sprite_height) {
          ...
          if (this.sprite_height === 16) {
            ...
            if (vertical_flip) {
              if (line < 8) sprite_tile_address += 16 + (~line & 0x07) * 2;
              else sprite_tile_address += (~(line - 8) & 0x07) * 2;
            } else sprite_tile_address += line * 2;
          } else {
            sprite_tile_address = tile_number * 16;
            sprite_tile_address += !vertical_flip ? line * 2 : (~line & 0x07) * 2;
          }
          if (this.is_cgb && tile_bank) sprite_tile_address += 0x2000;
          ...
          for (var x = sprite_x; x < sprite_x + 8; ++x) {
            ...
            if (
              ...
              if (color !== 0) {
                ...
                else sprite_buffer[x] = this.palette[this.dmg_spr_palettes[dmg_palette_number][color]];
              }
            }
          }
        }
...
GBC_video.prototype.mem_read = function (address) {
  if (address >= 0xff00) return this.reg_read(address);
  else if (address >= 0xfe00) return this.sprite_ram[address - 0xfe00];
  else return this.vram[this.vram_bank * 0x2000 + address - 0x8000];
};
GBC_video.prototype.mem_write = function (address, value) {
  if (address >= 0xff00) this.reg_write(address, value);
  else if (address >= 0xfe00) this.sprite_ram[address - 0xfe00] = value;
  else this.vram[this.vram_bank * 0x2000 + address - 0x8000] = value;
};
...
GBC_video.prototype.get_color_from_cgb_bkg_palette = function (palette, color) {
  var color_word =
    this.cgb_bkg_palettes[palette * 8 + color * 2] |
...
GBC_video.prototype.get_color_from_cgb_spr_palette = function (palette, color) {
  var color_word =
    this.cgb_spr_palettes[palette * 8 + color * 2] |
