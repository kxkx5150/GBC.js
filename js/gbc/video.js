...
function GBC_video(core)
{
   this.core = core;
   this.sprite_ram = new Uint8Array(160);
}

GBC_video.prototype.init = function(is_cgb)
{
   ...
   this.dmg_spr_palettes = [ [0, 0, 0, 0], [0, 0, 0, 0] ];
   
   // Fill the image with the default color to start off.
   this.clear_display();
   this.update_display();
   
   // Clear out our RAM, in case this is a reset.
...
GBC_video.prototype.clock = function(cycles)
{
   ...
   {
      // We've had enough cycles to advance the scanline.
   ...
   if (this.current_line < 144)
   {
      ...
      else
      {
         ...
         if (this.mode !== 0)
         {
            ...
            // If there's an HBlank DMA running, we need to run a step of it now.
            if (this.dma_running)
            {
               this.vram.set(this.dma_data.subarray(this.dma_bytes_done, this.dma_bytes_done + 0x10),
   ...
   else
   {
      ...
      if (this.mode !== 1)
      {
         // Switching into VBlank mode. Set the mode and fire the interrupt.
         this.mode = 1;

         // We always send this interrupt here, the core knows if it's enabled or not.
         this.core.raise_interrupt(0x40);
         // But only send the status interrupt if it's enabled.
         ...
         // And now update the display window.
         this.update_display();
      }
   }
...
GBC_video.prototype.draw_line = function()
{
   var background_buffer = [], sprite_buffer = [],
       background_pixels_set = [], background_priority = [];

   // We'll fill the line with the background color initially,
   //  and only do anything else if rendering is enabled.
   if (this.is_cgb)
   {
   ...
   if (this.background_enabled || this.is_cgb)
   {
      ...
      {
         var tile_pos = ((((this.current_line + this.scroll_y) & 0xff) >>> 3) * 0x20) + 
                        (this.scroll_x >>> 3) + tile_ix;
         
         // Don't wrap to the next line when crossing the right boundary of the map.
         ...
         var tile_bank_num = 0, horizontal_flip = 0, vertical_flip = 0, tile_priority = 0;
         if (this.is_cgb)
         {
         ...
         if (vertical_flip)
         {
            tile_data_lo = this.vram[(tile_bank_num * 0x2000) + this.background_tile_table - 0x8000 +
                                      (tile_offset * 16) +
                                      ((7 - ((this.current_line + this.scroll_y) & 0x07)) * 2)],
            tile_data_hi = this.vram[(tile_bank_num * 0x2000) + this.background_tile_table - 0x8000 +
                                      (tile_offset * 16) +
                                      ((7 - ((this.current_line + this.scroll_y) & 0x07)) * 2) + 1];
         }
         else
         {
            tile_data_lo = this.vram[(tile_bank_num * 0x2000) + this.background_tile_table - 0x8000 +
                                      (tile_offset * 16) + (((this.current_line + this.scroll_y) & 0x07) * 2)],
            tile_data_hi = this.vram[(tile_bank_num * 0x2000) + this.background_tile_table - 0x8000 +
                                      (tile_offset * 16) + (((this.current_line + this.scroll_y) & 0x07) * 2) + 1];
         }
   ...
   {
      // See if the window is even in a valid position first,
      //  and also whether the window is relevant to our current line.
      if ((this.window_x < 167) && (this.window_y < 144) && (this.window_y <= this.current_line))
      {
         // Now follow pretty much the same process as for the background,
         //  but don't start at x = 0, start at x = [window x position].
         for (var win_pixel = this.window_x - 7; win_pixel <= 166; win_pixel++)
         {
            // It's apparently possible for the X coordinate of the window to be set
            //  to a point off the screen. Don't try to draw off the screen.
            if (win_pixel < 0) continue;
            ...
            if (vertical_flip)
            {
               ...
               tile_data_hi = this.vram[(tile_bank_num * 0x2000) + this.background_tile_table - 0x8000 +
                                         (tile_offset * 16) +
                                         ((7 - ((this.current_line - this.window_y) & 0x07)) * 2) + 1];
            }
            else
            {
               tile_data_lo = this.vram[(tile_bank_num * 0x2000) + this.background_tile_table - 0x8000 +
                                         (tile_offset * 16) + (((this.current_line - this.window_y) & 0x07) * 2)],
               tile_data_hi = this.vram[(tile_bank_num * 0x2000) + this.background_tile_table - 0x8000 +
                                         (tile_offset * 16) + (((this.current_line - this.window_y) & 0x07) * 2) + 1];
            }
            
            var pixel_in_tile = horizontal_flip ? ((win_pixel - this.window_x + 7) & 0x07) :
   ...
   if (this.sprites_enabled)
   {
      ...
      for (var sprite_num = 0; sprite_num < 40; sprite_num++)
      {
         ...
         // See if this sprite's position is in range.
         if ((sprite_y > 0) && (sprite_y < 160) && (sprite_x < 168))
         {
            ...
            if ((this.current_line >= sprite_y) && (this.current_line < (sprite_y + this.sprite_height)))
            {
               ...
               // The sprite priority bit is inverted from what you would think.
               ...
               if (this.sprite_height === 16)
               {
                  ...
                  if (vertical_flip)
                  {
                     // Vertically flipping a 16 pixel sprite flips the entire sprite,
                     //  not each tile individually, so it's a bit tricky.
                     if (line < 8)
   ...
   // Copy our line buffer into the real image data object.
...
GBC_video.prototype.mem_read = function(address)
{
   if (address >= 0xff00)
...
GBC_video.prototype.mem_write = function(address, value)
{
   if (address >= 0xff00)
...
GBC_video.prototype.reg_read = function(address)
{
   if (address === 0xff40)
   {
      return this.background_enabled |
   ...
   {
      // Mode is always set to VBlank if the display is disabled.
      return this.mode | (this.coincidence_flag << 2) |
   ...
   {
      return this.dmg_spr_palettes[0][0] |
   ...
   {
      return this.dmg_spr_palettes[1][0] |
   ...
   {
      return ((this.dma_length_left / 16) - 1) |
             ((this.dma_running ? 0 : 1) << 7);
   }
...
GBC_video.prototype.reg_write = function(address, value)
{
   if (address === 0xff40)
   {
...
GBC_video.prototype.do_oam_dma = function(operand)
{
   if (typeof operand === "number")
   {
      // The program wants to do a DMA from inside video memory.
      this.sprite_ram = new Uint8Array(this.vram.subarray(operand, operand + 160));
   }
   else
   {
      // The source address was somewhere else, we've been given the data directly.
      this.sprite_ram = operand;
   }
};

GBC_video.prototype.do_vram_dma = function(data, destination, type)
{
   // This DMA unit doesn't exist on the DMG.
   if (this.is_cgb)
   {
      if (type)
      {
         // HBlank DMA; transfers 16 bytes during each H-blank period.
         this.dma_running = 1;
         this.dma_data = data;
      ...
      else
      {
         if (this.dma_running)
         {
            // The program wants to cancel a running Hblank DMA.
            this.dma_running = 0;
         }
         else
         {
            // General DMA; transfers everything all at once.
...
GBC_video.prototype.update_display = function()
{
   this.drawing_context.putImageData(this.image_data, 0, 0);
};

GBC_video.prototype.clear_display = function()
{
   for (var y = 0; y < 144; ++y)
      for (var x = 0; x < 160; ++x)
...
GBC_video.prototype.put_pixel = function(x, y, color)
{
...
GBC_video.prototype.get_color_from_cgb_bkg_palette = function(palette, color)
{
   var color_word =  this.cgb_bkg_palettes[(palette * 8) + (color * 2)] |
                    (this.cgb_bkg_palettes[(palette * 8) + (color * 2) + 1] << 8);
   return { r : (color_word & 0x1f) << 3,
            g : ((color_word >>> 5) & 0x1f) << 3,
            b : ((color_word >>> 10) & 0x1f) << 3 };
};

GBC_video.prototype.get_color_from_cgb_spr_palette = function(palette, color)
{
...
// These colors are intended to mimic the original Game Boy's green shades.
// The exact values are the defaults used in the emulator BGB,
//  which I highly recommend if you're ever doing GB development, as it is awesome.
GBC_video.prototype.palette = [
   {r:224, g:248, b:208}, // 0 = lightest color
   {r:136, g:192, b:112}, 
   {r: 52, g:104, b: 86},
   {r:  8, g: 24, b: 32}  // 3 = darkest color
];
